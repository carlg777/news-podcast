...
async function main() {
  ...
  const topics = process.env.INPUT_TOPICS
    ? JSON.parse(process.env.INPUT_TOPICS)
    : DEFAULT_TOPICS;
  const customQuery = process.env.INPUT_CUSTOM_QUERY || null;
  const articleCount = parseInt(
    process.env.INPUT_ARTICLE_COUNT || String(DEFAULT_ARTICLE_COUNT),
    10,
  );

  let id = podcastId;

  // Create podcast row outside the pipeline
  try {
    if (!id) {
  ...
  const steps = [
      ...
      fn: async (ctx) => {
        ...
        for (const topic of topics) {
          if (topic === "custom" && customQuery) {
            ...
            const customArticles = await searchGoogleNews(
              cleaned,
              articleCount,
            );
            allArticles.push(...customArticles);
          } else if (topic !== "custom") {
            const topicArticles = await fetchArticlesForTopic(
              topic,
              articleCount,
            );
    ...
    },
    {
      ...
      fn: async (ctx) => {
        ...
        console.log(
          `Collected ${sourceIds.length} source IDs of ${ctx.articles.length} articles`,
        );
    ...
    },
    {
      ...
      fn: async (ctx) => {
        ...
        console.log(
          `Podcast ${id} audio ready for download: ${audioUrl.slice(0, 80)}...`,
        );
      },
    },
    {
